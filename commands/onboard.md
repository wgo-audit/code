...
documentation-bearing paths in audited code roots, then delegate exactly one
documentation-preparation worker before asking to start the audit. Supply the
mandate, selected reviewers, repository mappings, cheap capability signals,
active audit platform, and selected same-platform catalog model; never request
or use another provider's credentials. It creates only the catalog and its
navigation signals; it does not ask intake questions or delegate further
workers. If it flags material missing or unresolved content, show the concise
list and ask whether the auditor wants to add sources. Ask for the sources only
after a yes, then continue the same worker and model against the expanded
